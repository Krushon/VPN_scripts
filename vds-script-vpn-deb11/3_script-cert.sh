#!/bin/bash
# Generate the necessary certificates for VPN. Генерируем необходимые сертификаты для VPN.
SECONDS=0
printf "\033c"
...
# Clarifying the necessary data. Уточняем необходимые данные
...
# Getting the name of the network interface. Получаем имя сетевого интерфейса (eth0, ens32, eno1 и т.д.)
net=`ip r | grep default | grep -Po '(?<=dev )(\S+)'`
# Getting the IP address of the network interface. Получаем ip-адрес сетевого интерфейса $net
vdsip=`ip addr show $net | awk '$1 == "inet" {gsub(/\/.*$/, "", $2); print $2}'`

# Distribute the received data by configs. Разносим полученные данные по конфигам
...
### Generating certificates. Генерируем сертификаты
cd $EASYRSAPATH
. ./vars
./easyrsa init-pki
# Creating a root certificate. Создаём корневой сертификат
(echo -en "\n\n") | ./easyrsa build-ca nopass # без пароля
./easyrsa build-ca
# Creating a Diffie-Hellman key. Создаём ключ Диффи-Хэлмана
./easyrsa gen-dh
# Creating a certificate request for the server and the certificate itself. Создаём запрос на сертификат для сервера и сам сертификат
(echo -en "\n") | ./easyrsa gen-req $company nopass
(sleep 1; echo -en "yes\n"; sleep 1; echo -en "\n") | ./easyrsa sign-req server $company nopass
# Creating a ta key. Создаём ta-ключ
openvpn --genkey --secret pki/$company-ta.key
# Creating a request for a client certificate and the certificate itself. Создаём запрос на клиентский сертификат и сам сертификат
(echo -en "\n") | ./easyrsa gen-req $company-user nopass
(sleep 1; echo -en "yes\n"; sleep 1; echo -en "\n") | ./easyrsa sign-req client $company-user nopass

# Copy the keys to the shared folder. Копируем ключи в общую папку
cp pki/ca.crt pki/$company-ca.crt
mkdir $KEYSPATH
cp -r pki/* $KEYSPATH

# Sorting server and user keys. Сортируем серверные и пользовательские ключи
mkdir $USERPATH
cd $KEYSPATH
...
# Create the ccd folder and the server.conf config. Создаём /ccd и server.conf
mkdir /etc/openvpn/ccd
touch /etc/openvpn/ccd/$company-user
#!!! SPECIFY YOUR SUBNETS. указываем СВОИ подсети !!!
...
# **** Setting up a firewall. Настраиваем файрвол ***************************************
...
# opening the tcp port for ssh connection. открываем tcp-порт для подключения по ssh
echo -en "add rule ip filter INPUT tcp dport $ssh counter accept\n" >> ~/ruleset.nft
# opening the port to connect the openvpn client. открываем порт для подключения клиента openvpn
echo -en "add rule ip filter INPUT $protocol dport 1194 counter accept\n" >> ~/ruleset.nft
#--- adding ip addresses of sip providers. добавляем ip-адреса sip-провайдеров ---#
# zadarma 
...
# in the range 10000:20000, a random port for sip conversation is generated
...
#-------------------------------------------#
# allow the ability to ping. оставляем возможность пинговать машину и пинговать с машины
...
# Setting up fail2ban. Настраиваем fail2ban
...
# ****Generating client configs. Генерация клиентских конфигов ******************************
cd $USERPATH
touch $company-openvpn.log #must be empty. должен быть пустой
...
tar -cvf $company.tar *
echo
echo "Client certificates and configs are generated (8 files) and packaged in the archive /etc/openvpn/user/$company.tar. It needs to be copied to the gateway."
